...
<div class="hidden md:flex md:w-1/2 items-center justify-center p-4">
                ...
                <div class="relative z-10 flex flex-col items-center justify-center h-full text-white p-10">
                    ...
                    <p id="video-subtitle" class="text-lg text-center max-w-sm" style="text-shadow: 0 0 10px rgba(0, 0, 0, 0.5);"></p>
                     <div class="absolute bottom-8 text-xs text-blue-200" style="text-shadow: 0 0 5px rgba(0, 0, 0, 0.5);">
                        &copy; 2025 Platform Akademik Awbit
                    </div>
                </div>
            </div>
        </div>
            ...
            <a href="{{ url('/') }}" class="flex items-center mb-6">
                ...
                <h1 class="text-xl font-bold text-gray-700">Platform Akademik Awbit</h1>
            </a>
            <h2 class="text-3xl font-bold text-gray-900">Login ke Akun Anda</h2>
            <p class="mt-2 text-sm text-gray-500">Silakan masukkan email dan kata sandi Anda.</p>
            
             <!-- Session Status: Menampilkan pesan seperti "password berhasil direset" -->
            @if (session('status'))
            ...
            <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-6">
                ...
                <div class="flex items-center justify-between">
                    ...
                    @if (Route::has('password.request'))
                        <a class="text-sm text-blue-600 hover:underline rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" href="{{ route('password.request') }}">
                            Lupa kata sandi?
                        </a>
                    @endif
                </div>
            ...
            <div class="mt-6 text-center">
                <p class="text-sm text-gray-500">
                    Belum punya akun?
                    <a href="{{ route('register') }}" class="text-blue-600 hover:underline font-semibold">Daftar sebagai Mahasiswa</a>
                </p>
            </div>
        </div>
    </div>
